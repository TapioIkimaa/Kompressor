...
fn setupTarget(b: *std.Build, step: *std.Build.Step, comptime tag: std.Target.Os.Tag, comptime arch: std.Target.Cpu.Arch, comptime kmpTarget: []const u8) !void {
    ...
    const libSuffix = switch (tag) {
        .windows => "lib1",
        else => "",
    };
    const lib = b.addLibrary(.{ .name = libPrefix ++ "z-jni", .linkage = .dynamic, .root_module = b.createModule(.{
    ...
    lib.root_module.addLibraryPath(b.path("../../build/nativebuilds/zlib-libz-jvm/jni/" ++ kmpTarget));
    lib.root_module.linkSystemLibrary(libPrefix ++ "z" ++ libSuffix, .{ .use_pkg_config = .no });
